...
export const bulkAddToWatchlist = async (watchlistId: string, isins: string[], category?: string) => {
  try {
    const response = await apiClient.post('/watchlist/bulk_add', {
      watchlist_id: watchlistId,
      isins: isins,
      category: category
    });
    ...
    // Return success count or boolean
    if (response.data && response.data.successful) {
      return response.data.successful.length;
    }
    return response.data && response.data.message ? 0 : 0;
  } catch (error) {
